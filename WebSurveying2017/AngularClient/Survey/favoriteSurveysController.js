(function (angular) {
    ...
    angular.module('WebSurveying2017').controller('favoriteSurveysController', fsCtrl);
    ...
    function fsCtrl(categoryService, surveyService, $stateParams, $state, $rootScope) {
        ...
        ctrl.pageChanged = function () {
            console.log('bbbbb')
            $state.go("searchFavotiteSurveys", { "page": ctrl.currentPage, "size": $stateParams.size, queryString: $stateParams.queryString, state: $stateParams.state });


        }

        ctrl.changeStateValue = function (state) {
            $state.go('searchFavoriteSurveys', { queryString: $stateParams.queryString, page: 1, size: 10, state: state });
        }

        ctrl.searchSurveys = function (queryString) {
            console.log(queryString);
            $state.go('searchFavoriteSurveys', { queryString: queryString, page: 1, size: 10, state: $stateParams.state });
