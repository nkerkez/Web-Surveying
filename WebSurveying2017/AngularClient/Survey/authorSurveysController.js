(function (angular) {
    ...
    angular.module('WebSurveying2017').controller('authorSurveysController', asCtrl);


    function asCtrl(categoryService, surveyService, $stateParams, $state, $rootScope) {
        var ctrl = this;
        ctrl.state = 'VIEW';
        ...
        ctrl.pageChanged = function () {
            console.log('bbbbb')
            $state.go("searchAuthorSurveys", { "page": ctrl.currentPage, "size": $stateParams.size, queryString: $stateParams.queryString, state: $stateParams.state });


        }

        ctrl.changeStateValue = function (state) {
            $state.go('searchAuthorSurveys', { queryString: $stateParams.queryString, page: 1, size: 10, state: state });
        }

        ctrl.searchSurveys = function (queryString) {
            console.log(queryString);
            $state.go('searchAuthorSurveys', { queryString: queryString, page: 1 , size : 10, state : $stateParams.state });
